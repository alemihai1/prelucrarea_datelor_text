...
import csv
req = urllib.request.Request("https://www.pcgarage.ro/audio-foto-video/", headers={"User-Agent": "Chrome"})
uclient = ureq(req)
page_html1 = uclient.read()
uclient.close()
page_soup1 = soup(page_html1, "html.parser")
subcategorii = page_soup1.findAll("small")
...
with open(filename, 'w', newline='', encoding="utf-16") as csvfile:
    ...
    req = urllib.request.Request("https://www.pcgarage.ro/", headers={"User-Agent": "Chrome"})
    uclient = ureq(req)
    page_html1 = uclient.read()
    uclient.close()
    page_soup1 = soup(page_html1, "html.parser")

    categorii = page_soup1.findAll("a", {"class": "cat-nav-tab"})

    for categ in categorii:
        if categ:
            req = urllib.request.Request(categ['href'], headers={"User-Agent": "Chrome"})
        uclient = ureq(req)
